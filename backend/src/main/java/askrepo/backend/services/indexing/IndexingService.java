...
import askrepo.backend.repository.RepositoryRepository;
import askrepo.backend.services.UserServices;
import askrepo.backend.services.ai.RagSettings;
import askrepo.backend.services.github.GitHubRateLimiter;
import askrepo.backend.services.github.GithubApiClient;
...
public class IndexingService {
    ...
    public Repository startIndexing(UUID repoId, UUID userId){
        ...
        if(repo.getIndexStatus()== IndexStatus.INDEXING){
            throw new BadRequestException("Repository is already beign indexed");
        }

        repo.setIndexStatus(IndexStatus.INDEXING);
        repo.setFileProcessed(0);
    ...
    private void doIndex(UUID repoId,UUID userId){
        Repository repo=repositoryRepository.findById(repoId)
                .orElseThrow(()->new NotFoundException("Repository not found"));
        String token=userServices.decryptAccessToken(userServices.getRequiredById(userId));

        deleteExistingVector(repoId.toString());
        ...
        for(String path:filePaths){
            try{
                ...
                List<Document> chunk=codeChunker.chunkFile(repoId.toString(),path,content);
                batch.addAll(chunk);
                totalChuncks+=chunk.size();
            ...
            if(processed% PROGRESS_EVERY_N_FILES==0 ||processed==filePaths.size()){
                updateProgress(repoId,filePaths.size(),processed,totalChuncks,IndexStatus.INDEXING,null);
            }
        ...
        markReady(repoId,filePaths.size(),processed,totalChuncks,repo.getFullName());
    ...
    private void deleteExistingVector(String repoId){
        try {
   ...
   @Transactional
    protected void updateProgress(
       ...
       repositoryRepository.findById(repoId).ifPresent(repo->{
           ...
           repo.setFileProcessed(processed);
   ...
   @Transactional
    protected void markReady(UUID repoId,int total,int processed,int chunks,String fullName){
       repositoryRepository.findById(repoId).ifPresent(repo->{
           repo.setFilesTotal(total);
           repo.setFileProcessed(processed);
           repo.setChunkCount(chunks);
       ...
       log.info("Indexing complete for {} ({} files, {} chunks)",fullName,processed,chunks);
   }
