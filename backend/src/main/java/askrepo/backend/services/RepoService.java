...
public class RepoService {
    ...
    public List<RepositoryResponse> syncAndListRepo(UUID userId){
        User user=userServices.getRequiredById(userId);
        String token=userServices.decryptAccessToken(user);
        List<Map<String,Object>> remoteRepos=githubApiClient.listUserRepo(token);
    ...
    @Transactional(readOnly=true)
    public Repository findByIdAndUserId(UUID repoId,UUID userId){
    ...
    public RepositoryResponse getRepo(UUID repoId,UUID userId){
        return toResponse(findByIdAndUserId(repoId,userId));
    }

    @Transactional(readOnly = true)
    public IndexStatusResponse status(UUID repoId,UUID userId){
        Repository repo=findByIdAndUserId(repoId,userId);
    ...
    public RepositoryResponse toResponseDto(Repository repo){
        return toResponse(repo);
    }

    private static RepositoryResponse toResponse(Repository repo){
